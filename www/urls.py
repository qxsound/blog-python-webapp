...
@view('manage_comments_list.html')
@get('/manage/comments')
def manage_comments():
...
def api_update_blog(blog_id):
    ...
    content = i.content.strip()
    if not name:
        raise APIValueError('name', 'name cannot be found')
    if not summary:
        raise APIValueError('summary', 'summary cannot be found')
    if not content:
        raise APIValueError('content', 'content cannot be found')
    blog = Blog.get(blog_id)
    if blog is None:
        raise APIResourceNotFoundError('Blog')
    blog.name, blog.summary, blog.content = name, summary, content
    blog.update()
    return blog
